...
def magneticStem(base,altura,B,P,v):
	'''
	This function is used when the particle get into the mushroom's stem and returns the position and the velocity
	of the particle when the particle scapes from the stem. It also plot the trayectory of the particle (so matplotlib required)
	INPUT:
	* base: base of the stem
	* altura: height of the stem
	* B: magnetic field
	* P: position of the particle when it get into the stem (x,y) coordinates
	* v: velocity of the particle when it get into the stem
	OUTPUT:
	The output is an array [P,v] with two elements. 
	P is the coordinates of the particle's position when it leaves the stem. It had two elements
	v is the velocity vector (vx,vy) when the particle leaves the stem.
	'''

	R = 1/B #radius

	tx = np.array([]) #plt.plot([-base/2,base/2],[0,0],'g')
	ty = np.array([]) #plt.plot([-base/2,-base/2,base/2,base/2],[0,-altura,-altura,0],'r')

	while True:
		#print(10*'*',i)
		
		#plt.plot(P[0],P[1],'ro')
		#plt.arrow(P[0],P[1],v[0]/10,v[1]/10)
	...
	return [P,v]#return [[tx,ty],v]
		#print(P,v)
#exit = magneticStem(1,1,1.5,np.array([-0.27873479,0.0]),np.array([-0.34643001,-0.56958818]))
#plt.show()
#print(exit[0])
#print(exit[1])
